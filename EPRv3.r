# 8/7/20 LSL Add GNU General Public license 
...
#  Copyright © 2020 Steve Bosworth, Anders Corr, and L. Stevan Leonard. All rights reserved.
...
external.data.name <-
  "epr-voter-data-v3.csv" # required name of input ballot data
sink(paste("EPRv3-", external.data.name, ".txt", sep="")) # send output to file
...
cat("\nThe program runs through the selector levels until all votes are counted, ")
cat("which might occur after any selector level is completed. Thus, when all the ")
cat("ballots that award an Excellent have been exclusively but provisionally counted ")
cat("for one of the candidates, the next rounds similarly count any “Very Goods” ")
cat("awarded on any remaining (uncounted) ballots, then “Goods”, and finally ")
cat("any Acceptables.\n")
...
while (more_voters) {
  ...
  cat("Count of affirmed evaluations at selector level:", selectors, "\n")
  ...
  if ((sum(selection) > 0) && (sum(candidate.vec1) > 0)) {
    ...
    # Note: Sum of ordinals will always be the same in Stage 1
        topWinnersByEvals <-
...
# Note: Although the order of candidates for the transfer does not change the number of votes each candidate
#   must transfer, the specific markings on ballots can change who gets a voter's vote 
    ...
    if (selector.max >= Acceptable) {
      ...
      print(paste("voter ",
                  ...
                  sep =
                    ""))
      if (length(candidates.equal.stage2) > 1) {
        cat("   Selected randomly from candidate ties: ", names[candidates.equal.stage2], "\n")
      }
...
cat(
  "In the case of ties for the last position in the order of candidates, the algorithm sums each tied candidate's ordinal evaluations, counted numerically, and will use this value first to break ties before one of the candidates with the same number of affirmed evaluations is chosen randomly.\n"
)
cat(
...
cat("Ballots that connot be transferred automatically become proxy votes of the candidate that was granted this ballot's highest grade in Stage 1, and are handled in Stage 4.\n")
# Note: Although the order of candidates for the transfer does not change the number of votes each candidate
#   must transfer, the specific markings on ballots can change who gets a voter's vote 
...
if (candidate.count > positions) {
  ...
  if (candidates.sorted[positions] == candidates.sorted[positions + 1]) {
    # first check the ordinal values of grades given to candidates to determine winner
    candidate.vec1[1:candidates] <- 0 # init all candidates 0
    # calculate ordinals by summing the grade given in original matrix
    # for voters who are assigned to the lowest candidates 
    for (j in positions:candidates) {
      this.candidate <- candidates.sorted.index[j]
      for (k in 1:70) {
        if (!is.na(voter.candidate[k]))
          if (voter.candidate[k] == candidate.names[this.candidate])
            candidate.vec1[this.candidate] <- candidate.vec1[this.candidate] + voter.mat.orig[k,this.candidate]
      }
    }
    lowest.candidates.ordinals <- which(candidate.vec1 != 0)
    cat("Ordinal values for lowest candidates (",
        names[lowest.candidates.ordinals], ") are ",
        candidate.vec1[lowest.candidates.ordinals], "\n")
    lowest.candidates.ordinals <- which(candidate.vec1 == max(candidate.vec1))
    if (length(lowest.candidates.ordinals) == 1) {
      cat("From tie of lowest candidates,",
          "the algorithm selected by highest ordinal candidate", 
          names[lowest.candidates.ordinals],
          "\n"
          )
      # put in terms of which of lowest is selected to match case with selector tie
      candidate.lowest <- lowest.candidates.ordinals - positions + 1
    }
    else {
      # in case of tie in both count of selectors and sum of oridinal values, choose a winner randomly from top ordinal winners
      # sample() returns random choice from equal winners
      ...
      candidate.lowest <-
        sample(which(candidates.to.narrow == max(candidates.to.narrow)), size =
                 1)
      cat(
        "From tie of lowest candidates, ordinal values for candidates were the same,",
        "so the algorithm randomly selected candidate",
  ...
  while ((max.weight <- max(weights.stage3.transfer)) > 0) {
    ...
    print(
      paste(
        "Transfer ",
        transfer.count,
        " votes (remaining highest affirmed evaluations) on ballots currently held by unelected candidate ",
        transfer.name,
        sep = ""
      )
    )
    ...
    voters.to.transfer <- NULL
    voter.candidate.to.transfer <- NULL
    # select voters that match candidate to transfer and have not previously been marked for transfer
    for (i in 1:voters) {
      if (is.na(voter.candidate[i]))
        next()
      if ((voter.candidate[i] == transfer.name) &&
    ...
    for (i in 1:voter.count.stage3) {
      ...
      if (selector.max >= Acceptable) {
        ...
        if (length(candidates.equal.stage3) > 1) {
          cat("   Selected from candidate ties: ", names[candidates.equal.stage3], "\n")
        }
    ...
    if (proxy.required) {
      cat(
        ...
        transfer.count,
        " proxy vote(s) must be transferred in Stage 4 by this candidate ",
        "or by whichever candidate was given this ballot in Stage 1.\n", sep=""
      )
      voters.to.transfer <- NULL
      # select voters that match candidate to transfer
      for (i in 1:voters) {
        if (is.na(voter.candidate[i]))
          next()
        # include candidates transferred previously to select original candidate from Stage 1 for proxy vote 
        if (voter.candidate[i] == transfer.name) 
          voters.to.transfer <- c(voters.to.transfer, i)
      }
      cat("Voters' ballots to be transferred:",
          voters.to.transfer,
          "\n")
      xfer.proxy.count <-
        min(length(voters.to.transfer), transfer.count)
      # update voter array
      # for each voter, select the candidate from stage 1 to hold the proxy vote
      transfer.this.name <- "candidate to do the transfer"
      for (vtt in 1:transfer.count) {
        if (!is.na(voter.transfer[voters.to.transfer[vtt]]))
        {
          # extract candidate from Stage 1, which is X in the transfer string "From X to Y"
          transfer.this.name <- substr(voter.transfer[voters.to.transfer[vtt]], 6, 6) 
          cat("Proxy vote for voter ",
            voters.to.transfer[vtt],
            " will be transferred by candidate from Stage 1: ", 
            transfer.this.name,
            "\n")
        }
        else
        {
          transfer.this.name <- transfer.name
          cat("Proxy vote for voter ",
              voters.to.transfer[vtt],
              " will be transferred by the candidate that was granted this ballot's highest grade in Stage 1: ", 
              transfer.this.name,
              "\n")
        }
        voter.transfer[voters.to.transfer[vtt]] <-
        paste("Proxy vote Stage 3 from ", transfer.this.name, sep = "")
      }
      # proxy votes end the transfer phase for this candidate
      # update weights to reflect proxy votes that will occur later
      weights.stage3[transfer.candidate] <-
